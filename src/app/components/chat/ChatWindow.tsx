...
export default function ChatWindow({ initialConversationId }: ChatWindowProps) {
    ...
    useEffect(() => {
        if (
            error?.includes("Conversation not found") ||
            error?.includes("404")
        ) {
            ...
            createNewConversation()
                ...
                .catch((err) => {
                    console.error(
                        "Failed to auto-recover with new conversation:",
                        err
                    );
                    setRecoveryInProgress(false);
    ...
    useEffect(() => {
        ...
        if (prevConversationIdRef.current && !initialConversationId) {
            console.log(
                "Conversation ID cleared, checking for existing conversations"
            );
            setRecoveryInProgress(true);
            ...
            fetch("/api/conversations?include_archived=false")
                ...
                .then((data) => {
                    ...
                    if (data.conversations && data.conversations.length > 0) {
                        console.log(
                            "Found existing conversations, not creating a new one:",
                        ...
                        console.log(
                            "No existing conversations found, creating a new one"
                        );
    ...
    const handleRecovery = async () => {
        try {
            ...
            const response = await fetch(
                "/api/conversations?include_archived=false"
            );
    ...
    const showWelcomeState =
        !currentConversation && !isLoading && !recoveryInProgress;
            ...
            <div className="flex-1 p-6 overflow-y-auto space-y-6 bg-gradient-to-b from-gray-950 to-black">
                ...
                {error && showRecoveryButton && (
                    <div className="p-3 bg-red-900/70 text-red-100 rounded-lg text-sm border border-red-800 animate-fadeIn">
                        ...
                        <p>
                            We encountered an issue loading your conversation.
                        </p>
                        <button
