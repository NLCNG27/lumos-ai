...
export const addDocument = async (
  ...
  try {
    ...
    const { documentId, documents } = processDocument(fileContent, fileName);
    ...
    await initializeVectorStore(documentId, documents);
...
// Get document info
export const getDocumentInfoById = async (documentId: string): Promise<DocumentInfo | null> => {
  try {
...
export const queryDocument = async (
  ...
  try {
    ...
    if (!vectorStore) {
      throw new Error(`Vector store for document ${documentId} not found`);
    }
