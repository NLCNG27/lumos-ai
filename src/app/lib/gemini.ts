...
export const DEFAULT_SYSTEM_MESSAGE = `You are Lumos, a helpful AI assistant that can help with general tasks, while 
also being able to analyze and understand complex topics.
...
export function getGeminiModel(
    ...
    const finalModel = Object.values(GEMINI_MODELS).includes(
        modelName as string
    )
        ? modelName
 ...
 */
export function getGeminiModelWithCodeExecution(
    modelName = GEMINI_MODELS.GEMINI_FLASH
) {
    ...
    console.log(
        `Creating Gemini model with code execution capability: ${finalModel}`
    );
    ...
    try {
        ...
        console.error(
            `Failed to create Gemini model with code execution: ${error}`
        );
        throw error;
    }
}
            ...
            for (const item of message.content) {
                if (item.type === "text") {
                    ...
                    if (item.image_url.url.startsWith("data:")) {
                        ...
                        const matches = item.image_url.url.match(
                            /^data:([^;]+);base64,(.+)$/
                        );
...
export async function callGeminiWithTimeout(
    ...
    try {
        ...
        if (
            ...
            console.log(
                "Falling back with reduced token count but still using gemini-2.0-flash"
            );
...
export async function generateMultimodalResponse(
    ...
    try {
        ...
        const hasImageParts = geminiContents.some(
            (content) =>
                content.parts &&
        ...
        if (hasImageParts) {
            console.log(
                "Processing request with image content using gemini-2.0-flash"
            );
        }
...
export async function verifyGeminiApiKey(
    ...
    try {
            ...
            contents: [
                {
                    ...
                    parts: [
                        {
                            text: "Hello, this is a test message to verify API key access.",
                        },
                    ],
                },
            ],
        ...
        console.error(
            `Failed to verify Gemini API key for model ${modelName}:`,
            error
        );
