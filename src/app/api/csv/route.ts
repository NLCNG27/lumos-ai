...
const processCSVForChart = (parsedData: { headers: string[], data: any[] }) => {
  ...
  if (needsDownsampling) {
    ...
    for (let colIndex = 1; colIndex < Math.min(headers.length, MAX_COLUMNS); colIndex++) {
      const datasetKey = headers[colIndex];
      const dataPoints = new Array(Math.ceil(data.length / downsampleFactor));
      
      // For each downsampled point
      for (let i = 0; i < labels.length; i++) {
        const startIdx = i * downsampleFactor;
        const endIdx = Math.min(startIdx + downsampleFactor, data.length);
        
        // Take the label from the first point in the group
        if (colIndex === 1) { // Only need to do this once
...
const generateSampleData = () => {
  ...
  const values1 = Array.from({ length: 12 }, () => Math.floor(Math.random() * 100));
  const values2 = Array.from({ length: 12 }, () => Math.floor(Math.random() * 80));
